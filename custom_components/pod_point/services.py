        ...
        schema=vol.Schema(
            {
                ...
                vol.Optional(ATTR_HOURS): vol.All(vol.Coerce(int), vol.Range(min=0,max=24)),
                vol.Optional(ATTR_MINUTES): vol.All(vol.Coerce(int), vol.Range(min=0,max=59)),
                vol.Optional(ATTR_SECONDS): vol.All(vol.Coerce(int), vol.Range(min=0,max=59))
            }
        ),
    )
...
async def get_coordinator(hass: HomeAssistant, config_entry_id: str) -> PodPointDataUpdateCoordinator:
...
async def handle_charge_now(
    ...
    if len(pods) == 1:
        ...
        PodPointServiceException(f"Service only supports accounts with 1 Pod attached, found {len(pods)} Pods!")
    ...
    if valid_time_passed is False:
        raise PodPointServiceException("Please pass an hours, minutes or seconds value. Cannot set 'charge now' with 0 values.")

    await api.async_set_charge_override(pod=pod, hours=hours, minutes=minutes, seconds=seconds)
